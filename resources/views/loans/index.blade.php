...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
    ...
    <div class="bg-white shadow sm:rounded-lg mb-6 p-4 flex items-center justify-between">
        <div>
            <h3 class="text-sm font-medium text-gray-900">Pengaturan Bunga Koperasi</h3>
            <p class="text-xs text-gray-500">Nilai bunga default untuk setiap pengajuan pinjaman baru.</p>
        </div>
        <form action="{{ route('loans.settings.interest') }}" method="POST" class="flex items-center gap-3">
            @csrf
            <div class="relative rounded-md shadow-sm w-32">
                <input type="number" step="0.01" name="default_interest_rate" id="default_interest_rate" class="focus:ring-indigo-500 focus:border-indigo-500 block w-full py-2 pl-3 pr-8 sm:text-sm border-gray-300 rounded-md" value="{{ $defaultInterestRate }}" required>
